...
void InializarCuerposHexagono(body bodies[])
{    int i,j,aux=0; 
     double sx=L/4.0; //Separación horizontal de las moleculas
     double sy= sx*sqrt(3.0)/2.0; //Altura del hexágono 
     for (i=0; i<N; i++)
     {bodies[i].x=L/8.0+sx*(i%4)-L/8.0*(aux%2);
      bodies[i].y=L/8.0+L/4.0*aux;
      bodies[i].vx=0;
      bodies[i].vy=0;
      bodies[i].ax=0;
      bodies[i].ay=0;
